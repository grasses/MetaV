...
class RP():
    ...
    def rp(self, x):
        ...
        x = kornia.geometry.transform.resize(x, size=(rnd, rnd))
...
class GaussianBlur():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            x = kornia.filters.gaussian_blur2d(x,  (self.ksize, self.ksize), (self.sigma, self.sigma))
        return x
...
class AverageBlur():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            x = kornia.filters.blur.box_blur(x,  (self.ksize, self.ksize))
        return x
...
class MedianBlur():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            x = kornia.filters.median_blur(x, (self.ksize, self.ksize))
        return x
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            if np.random.rand() < 0.5:
                angle = -angle
            x = kornia.geometry.transform.rotate(x, angle=angle.to(x.device, dtype=torch.float32))
        return x
...
class HFlip():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            x = kornia.geometry.transform.flips.hflip(x)
        return x
...
class VFlip():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            x = kornia.geometry.transform.flips.vflip(x)
        return x
...
class RandShear():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            x = kornia.geometry.transform.shear(x, shear.to(device=x.device, dtype=x.dtype))
        return x
...
class RandTranslate():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            x = kornia.geometry.transform.translate(x, translate)
        return x
...
class NonLocalMean():
    ...
    def __call__(self, x):
        ...
        if np.random.rand() < self.p:
            ...
            datas = kornia.color.rgb_to_bgr(x.clone())
            datas = kornia.utils.tensor_to_image(datas)
            ...
            datas = kornia.utils.image_to_tensor(res_list)
            datas = kornia.color.rgb.bgr_to_rgb(datas)
...
class MedianBlur_CV():
    ...
    def __call__(self, x):
        ...
        if np.random.rand() < self.p:
            ...
            datas = kornia.color.rgb.rgb_to_bgr(x.clone())
            datas = kornia.utils.tensor_to_image(datas)
...
class MedianBlur_AUG():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            datas = kornia.utils.tensor_to_image(x)
            datas = np.round(datas * 255)
            datas = datas.astype(np.uint8)

            datas = self.aug(images=datas)
            datas = kornia.utils.image_to_tensor(datas)
            datas = datas.float() / 255
...
class CropResizePad_AUG():
    ...
    def __call__(self, x):
        if np.random.rand() < self.p:
            ...
            datas = kornia.utils.image_to_tensor(np.array(datas))
            datas = datas.float() / 255
